...
class TwitterAdapter(SourceAdapter):
    def __init__(self, queries: List[str] = None):
        if not queries:
            self.queries = ["myntra cart", "myntra wishlist", "help me choose myntra"]
        else:
    ...
    def platform_name(self) -> str:
        return SourcePlatform.TWITTER.value
    ...
    def _sync_fetch(self, limit: int) -> List[Dict[str, Any]]:
        run_input = {
            "searchTerms": self.queries,
            "maxItems": limit,
        }
        try:
            print(f"[{self.platform_name}] Starting Apify Twitter scraper...")
    ...
    def normalize(self, raw_data: List[Dict[str, Any]]) -> List[dict]:
        ...
        for item in raw_data:
            ...
            normalized.append({
                ...
                "author_id_anonymized": Normalizer.generate_content_hash(item.get("author", {}).get("userName", "anonymous"), "author", ""),
